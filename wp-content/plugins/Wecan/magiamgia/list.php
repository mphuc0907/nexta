...
global $wpdb;
$list_voucher = $wpdb->get_results("SELECT * FROM `voucher` ");
            ...
            foreach ($myrows as $key => $item) {
                ...
                <tr>
                    ...
                    <?php } else { ?>
                        ...
                        <?php } else { ?>
                            <td><?= number_format($item->max_discount, 0, ',', '.') . ' VND' ?></td>
                        <?php } ?>
                    <?php } ?>
                    <td><?= $item->number_of_vouchers ?> </td>
                    <td><a href="javascript:void(0)" class="remove-action" data-id="<?= $item->id ?>">Ẩn</a></td>
