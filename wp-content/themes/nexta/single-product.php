            ...
            <?php foreach ($news as $key => $value)  :
                if ($value->ID != $id) {
                    ...
                    <div class="item <?= $key > 1 ? 'col-lg-6 col-md-6' : 'col-lg-4 col-md-6' ?>">
                        ...
                        <div class="action">
                            <a href="javascript:;" class="btn btn-add-cart add-to-cart book_now book_now_<?= $value->ID ?>" data-qty="1" data-maxqty="<?= get_field('qty',$value->ID) ?>" data-id="<?= $value->ID ?>" data-maxqty="<?= get_field('qty',$id) ?>" data-title="<?= $value->post_title ?>" data-text="add-cart" data-link="<?php echo get_permalink($value->ID) ?>" data-id="<?= $value->ID ?>" data-img="<?php echo checkImage($value->ID) ?>" data-price="<?= get_field('price',$value->ID) ?>">Thêm vào giỏ hàng</a>
